...
(function () {
    ...
    function wireListen(root, splitter) {
        ...
        nodes.forEach((textNode) => {
            ...
            parts.forEach((part) => {
                frag.appendChild(
                    part.lang
                        ? listenSpan(part.text, part.lang)
        ...
        return function (textNode) {
            ...
            text.split(SEGMENT_SEP).forEach((chunk, i) => {
                ...
                if (
                    i % 2 === 1 ||
                    !core ||
                    LABEL_ONLY.test(core) ||
                    !looksTargetLanguage(core)
                ) {
    ...
    function detectTargetLang() {
        ...
        document
            .querySelectorAll(".exercise[data-audio-lang]")
            .forEach((ex) => {
    ...
    function renderMarkdown() {
        ...
        const latinSplitter =
            targetLang && !targetLang.startsWith("ru")
                ? makeLatinSplitter(targetLang)
                : null;
    ...
    function setupQuizzes() {
        document
            .querySelectorAll('.exercise[data-type="quiz"]')
            .forEach((ex) => {
                ...
                opts.forEach((opt) => {
                    ...
                    audioBtn.dataset.audioLang = optionAudioLang(
                        opt,
                        audioLang,
                    );
    ...
    const OPTION_PT_HINT =
        /[ãõçáéíóúâêôà]|\b(não|que|para|uma|um|dos|das|pelo|pela|sem|mais|ou|se|quando|porque|onde|qual|quem|isso|esse|essa|eu|ele|ela|eles|elas|vou|tenho|muito|obrigado|obrigada|de|da|do|em|na|os)\b/i;
    ...
    function optionAudioLang(opt, exerciseLang) {
        ...
        if (
            exerciseLang.startsWith("en") &&
            OPTION_PT_HINT.test(opt.replace(/\([^)]*\)/g, ""))
        )
            return "pt-BR";
        return exerciseLang;
    }
    ...
    function setupCyrillicKeyboards() {
        document.querySelectorAll("[data-cyrillic-keyboard]").forEach((kb) => {
            ...
            const input = exercise
                ? exercise.querySelector(".text-input")
                : null;
            if (!input) return;
    ...
    const codeExercisesCount = document.querySelectorAll(
        '.exercise[data-type="code"], .exercise:not([data-type])',
    ).length;
    ...
    function ensurePyodide() {
        if (!pyodidePromise) {
            pyodidePromise = loadPyodide().then((py) => {
                if (statusEl) {
                    statusEl.innerHTML =
                        '<i class="ph-duotone ph-check-circle"></i> Ambiente Python pronto!';
    ...
    function runSqlQuery(SQL, setupSql, query) {
        ...
        if (statementCount > 1) {
            throw {
                userError:
                    "Escreva uma única consulta SELECT (sem múltiplos comandos separados por ';').",
            };
        }
        ...
        try {
            ...
            if (result.length === 0) {
                throw {
                    userError:
                        "Sua consulta não retornou nenhuma tabela de resultado — use um SELECT.",
                };
            }
    ...
    function runSqlStatementAndVerify(SQL, setupSql, statement, verifyQuery) {
        ...
        if (statementCount > 1) {
            throw {
                userError:
                    "Escreva um único comando (sem múltiplos comandos separados por ';').",
            };
        }
        ...
        try {
            ...
            if (result.length === 0) {
                throw {
                    userError: null,
                    raw: new Error(
                        "A consulta de verificação não retornou dados.",
                    ),
                };
            }
            const { columns, values } = result[0];
    ...
    function compareSqlResults(a, b, orderMatters) {
        ...
        const stringifyRow = (row) =>
            JSON.stringify(row.map((v) => (v === null ? " NULL" : String(v))));
    ...
    function resolveSetupSql(ex) {
        ...
        const setupSql =
            (opts && typeof opts === "object" && opts.setup_sql) ||
            topicSetupSql;
    ...
    const scoreViews = [
        scorePanel,
        document.getElementById("score-float"),
    ].filter(Boolean);
    ...
    function registerMiss(exId) {
        if (!passed.has(exId))
            wrongTries.set(exId, (wrongTries.get(exId) || 0) + 1);
    }
    ...
    function autoSaveScore() {
        ...
        Progress.markDone(
            parseInt(completeBtn.dataset.topicId, 10),
            score,
        ).catch(() => {});
    }
    ...
    function sameAnswer(a, b) {
        return (
            ...
            looseNormalize(a) === looseNormalize(b)
        );
    }
    ...
    function buildTextErrorFeedback(student, solution) {
        ...
        const correctWords = wordsStudent.filter((w) =>
            wordsSolution.includes(w),
        );
        const missingWords = wordsSolution.filter(
            (w) => !wordsStudent.includes(w),
        );
        ...
        if (student.trim() === "") {
            ...
            hint +=
                "A resposta está bem diferente. Ouça novamente ou reveja a lição.";
        }
        return hint;
    }
    ...
    async function runExercise(ex) {
        ...
        const solution = ex
            .querySelector("[data-solution-code]")
            .textContent.trim();
        ...
        if (type === "text" || type === "audio") {
            ...
            if (sameAnswer(studentAnswer, solution)) {
                ...
                output.classList.add("err");
                output.textContent = buildTextErrorFeedback(
                    studentAnswer,
                    solution,
                );
                registerMiss(exId);
        ...
        if (type === "quiz") {
            ...
            if (sameAnswer(selected.textContent, solution)) {
                ...
                options.forEach((b) => {
                    if (sameAnswer(b.textContent, solution))
                        b.classList.add("is-correct");
                });
                output.classList.add("err");
                output.textContent =
                    "Não foi dessa vez. A opção correta está destacada.";
                registerMiss(exId);
            }
            onAnswerChecked();
            return;
        ...
        if (type === "sql") {
            ...
            try {
                ...
                output.classList.add("err");
                output.textContent =
                    "Não foi possível carregar o motor de SQL. Verifique sua conexão.";
                return;
            }
            attempted.add(exId);
            ...
            try {
                ...
                output.classList.add("err");
                output.textContent =
                    "Erro interno no exercício (comando de referência falhou). Avise quem criou o curso.";
                return;
            }

            if (
                compareSqlResults(studentResult, solutionResult, orderMatters)
            ) {
                output.classList.add("ok");
        ...
        try {
            ...
            output.classList.add("err");
            output.textContent =
                "Não foi possível carregar o ambiente Python. Verifique sua conexão.";
            return;
        }
        attempted.add(exId);
...
`);
    ...
    function formatPyError(message) {
        ...
        if (last.startsWith("AssertionError")) {
            ...
            return (
                msg || "O resultado não passou na verificação. Tente de novo."
            );
        }
        return last;
    }
    ...
    function startSpeakExercise(ex) {
        ...
        if (!SpeechRecognition) {
            ...
            resultEl.textContent =
                "Seu navegador não suporta reconhecimento de voz. Use o Chrome ou o Edge.";
            return;
        }
        ...
        recognition.onerror = (event) => {
            ...
            if (event.error === "no-speech") {
                resultEl.textContent =
                    "Nenhuma fala detectada. Fique mais perto do microfone.";
        ...
        recognition.onend = () => {
            btn.innerHTML =
                '<i class="ph-duotone ph-microphone"></i> Gravar fala';
            btn.disabled = false;
        };
    }
        ...
        document.querySelectorAll(".exercise").forEach((ex) => {
            ...
            solBtn.addEventListener("click", () => {
                ...
                const showSolution =
                    hasAttempted ||
                    confirm(
                        "Mostrar a solução? Tente resolver sozinho primeiro 🙂",
                    );
                if (showSolution) {
                    ...
                    if (type === "code" || type === "sql") {
                        ...
                        ex.querySelectorAll(".quiz-option").forEach((b) => {
                            ...
                            b.classList.toggle(
                                "is-correct",
                                sameAnswer(b.textContent, solution),
                            );
                        });
                    } else {
            ...
            audioBtns.forEach((btn) => {
                btn.addEventListener("click", () => {
                    speak(
                        btn.dataset.audioText,
                        btn.dataset.audioLang || "en-US",
                        parseFloat(btn.dataset.speed || "1"),
                    );
                });
            });
            ...
            completeBtn.addEventListener("click", async () => {
                ...
                try {
                    ...
                    completeBtn.innerHTML =
                        '<i class="ph-duotone ph-check"></i> Concluído';
                    completeBtn.disabled = false;
                } catch (e) {
